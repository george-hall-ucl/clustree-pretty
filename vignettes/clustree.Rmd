...
By default the tree is drawn using the `"clustree"` layout. Each clustering is
placed on its own row, with the lowest resolution at the top, and the clusters
in a row are evenly spaced and centred. Because the spacing is constant, rows
containing more clusters are wider than those containing fewer, which gives the
tree a symmetrical outline.

The clusters in each row are also ordered. Working down the tree, each cluster
is placed under the cluster at the previous resolution that contributes the
greatest proportion of itself to it, and the clusters sharing a parent are
sorted by how much of that parent they represent. This keeps related clusters
next to each other and reduces the number of crossing edges. To place the
clusters in sorted order instead, set `order_clusters` to `FALSE`.

```{r nba-layout-unordered}
clustree(nba_clusts, prefix = "K", order_clusters = FALSE)
```

Each row is labelled with the clustering it shows. These labels can be removed
by setting `show_res_labels` to `FALSE`, which also brings back the node colour
legend.

```{r nba-layout-nolabels}
clustree(nba_clusts, prefix = "K", show_res_labels = FALSE)
```

The tree can also be drawn using one of the layout algorithms provided by the
`igraph` package. The Reingold-Tilford tree layout algorithm tries to place
nodes below their parents [@Reingold1981-iy], while the Sugiyama layout tries to
minimise the number of crossing edges [@Sugiyama1981-wu].
...
change the colour scale used for the nodes:
...
Edges are drawn in a single colour, set using the `edge_colour` argument, and
their width shows the proportion of a cluster that comes from each of the
clusters above it:

```{r modify-edges}
clustree(nba_clusts, prefix = "K", edge_colour = "steelblue", edge_width = 3)
```
...
fit better. For example let's remove the node size legend and move the rest to
the bottom:
